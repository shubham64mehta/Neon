...
class _SolidColorState extends State<SolidColor1> with SingleTickerProviderStateMixin {
...
Future<List> read()async {
  FirebaseDatabase.instance.reference().child("Usermode").child(user1).once().then((DataSnapshot snapshot){
...
//Baad mai small case se name karna. Nahi toh galat progrramming practice hai
    ...
    values.forEach((key,value){
       FirebaseDatabase.instance.reference().child("Usermode").child(user1).child(key).child("hexcolor").once().then((DataSnapshot s){//run
          ...
          return AnimatedBuilder(
            ...
            builder: (BuildContext context ,Widget child){
              return  Transform(
               ...
               transform: Matrix4.identity()..setEntry(3, 2, 0.005)..rotateX(2*pi*animation.value),
...
Future<void> add(var color1 ,var color2) async{
  ...
  DatabaseReference  _color2 = database.reference().child("Usergradient").child(user1).child(uuid);
