...
class _GradientState extends State<Gradient1> with SingleTickerProviderStateMixin{
  AnimationController _animationController;

Future<String>read()async {
  FirebaseDatabase.instance.reference().child("Gradient").once().then((DataSnapshot snapshot){
//should be "user"
//Baad mai small case se name karna. Nahi toh galat progrramming practice hai
    ...
    values.forEach((key,value){
       FirebaseDatabase.instance.reference().child("Gradient").child(key).child("Hexcolor1").once().then((DataSnapshot s){//run
         gradient1.add(Color(s.value));//array of hexcolor1
      
  });
   FirebaseDatabase.instance.reference().child("Gradient").child(key).child("hexcolor2").once().then((DataSnapshot f){//run
